"""
GitHub Discussion Feature Node.

Extracts discussion/comment metrics:
- Commit comments
- PR comments
- Issue comments
...
class GitHubDiscussionNode(FeatureNode):
    """
    Fetches discussion metrics from GitHub API.
    
    Counts:
    - Comments on commits in this build
    - PR review comments (if applicable)
    - Related issue comments
    - PR description word count (complexity)
    """
    
    def extract(self, context: ExecutionContext) -> Dict[str, Any]:
        gh_handle: GitHubClientHandle = context.get_resource(ResourceNames.GITHUB_CLIENT)
