...
class RepoSnapshotNode(FeatureNode):
    ...
    def extract(self, context: ExecutionContext) -> Dict[str, Any]:
        ...
        with repo_lock(str(repo.id)):
            snapshot_metrics = self._analyze_snapshot(repo_path, effective_sha, language)
    ...
    def _get_history_metrics(self, repo_path: Path, sha: str) -> Tuple[float, int]:
        ...
        try:
            ...
            first_commit_ts = subprocess.run(
                ...
                for path in worktree_path.rglob("*"):
                    ...
                    try:
                        ...
                        if _is_test_file(rel_path, language):
                            ...
                            for line in lines:
                                clean_line = _strip_comments(line, language or "")
                                if _matches_test_definition(clean_line, language):
                                    test_cases += 1
                                if _matches_assertion(clean_line, language):
                                    asserts += 1
